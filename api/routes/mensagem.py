...
@router.post("/", response_model=MensagemSchemas)
async def registrar_mensagem(mensagem: MensagemSchemas, db: AsyncSession = Depends(get_db)):
    service = MensagemService(db)
    return await service.registrar_mensagem(mensagem)

@router.get("/partida/{partida_id}", response_model=list[MensagemSchemas])
