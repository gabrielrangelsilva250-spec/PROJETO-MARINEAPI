...
@router.post("/", response_model=TurnosSchema)
def registrar_turno(turno: TurnosSchema, db: Session = Depends(get_db)):
    service = TurnosService(db)
    return service.registrar_turno(turno)

@router.get("/partida/{partida_id}", response_model=list[TurnosSchema])
def listar_turnos_por_partida(partida_id: int, db: Session = Depends(get_db)):
    service = TurnosService(db)
    return service.listar_por_partida(partida_id)

@router.get("/partida/{partida_id}/ultimo", response_model=TurnosSchema)
